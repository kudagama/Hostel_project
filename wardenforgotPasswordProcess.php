...
if (isset($_GET["e"])) {
    ...
    $rs = Database::search("SELECT * FROM `warden` WHERE `email`='" . $email . "' ");
    $n = $rs->num_rows;
    ...
    if ($n == 1) {
        ...
        Database::iud("UPDATE `warden` SET `verification_code`='" . $code . "' WHERE `email`='" . $email . "' ");
        ...
        $bodyContent = '<h3 style="color:black">Hello Warden,</span></h3>
