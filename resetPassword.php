...
if (empty($email)) {
    ...
    $rs = Database::search("SELECT * FROM `landloaders` WHERE `email`='" . $email . "' AND `verification_code`='" . $vCode . "' ");
    $n = $rs->num_rows;

    if ($n == 1) {

        Database::iud("UPDATE `landloaders` SET `password`='" . $newPassword . "' WHERE `email`='" . $email . "' ");
        if (isset($_SESSION["u"])) {
